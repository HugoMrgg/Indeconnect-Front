...
export function ProductReviewsSection({ productId, canReview = false }: Props) {
    ...
    const handleDisableReview = async (reviewId: number) => {
        if (!confirm(t('products.reviews.disable_confirm'))) return;

        try {
            await disableProductReview(reviewId);
            toast.success(t('products.reviews.disabled_success'));
            ...
            toast.error(t('products.reviews.disable_error'));
        }
    };
                    ...
                    <h2 className="text-2xl font-bold flex items-center gap-3">
                        ...
                        {reviews.length > 0 && (
                            <span className="text-sm font-normal text-gray-600 bg-gray-100 px-3 py-1 rounded-full">
                                {t('products.reviews.rating').replace('★', `★ ${averageRating}`)}
                            </span>
                        )}
                    </h2>
                    <p className="text-gray-500 text-sm mt-1">
                        {reviews.length} {reviews.length > 1 ? t('products.reviews.comment_plural') : t('products.reviews.comment_singular')}
                    </p>
                </div>
            ...
            <div className="space-y-6">
                {loading ? (
                    ...
                    <div className="bg-gray-50 rounded-xl p-8 text-center border border-dashed border-gray-200">
                        <p className="text-gray-500">{t('products.reviews.no_reviews')}</p>
                    </div>
                ) : (
                    reviews.map((r) => (
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    {canModerate && (
                                        <button
                                            ...
                                            title={t('products.reviews.disable_button')}
